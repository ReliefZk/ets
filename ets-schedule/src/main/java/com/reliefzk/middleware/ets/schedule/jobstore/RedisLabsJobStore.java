 ...
 * @author Matan Kehat
 */
public class RedisLabsJobStore implements JobStore {
    ...
    public void initialize(ClassLoadHelper loadHelper, SchedulerSignaler signaler) throws SchedulerConfigException {
        ...
        logger.info("init pool = {}:{}", host, port);
        if (password != null) {
        ...
        logger.info("init lockPool = {}:{}", host, port);
        if (password != null) {
    ...
    public void schedulerStarted() throws SchedulerException {
        if (this.instanceIdFilePath != null) {
            ...
            if(!instanceIdFile.exists()){
                try {
                    instanceIdFile.createNewFile();
                } catch (IOException e) {
                    logger.error("create file error!", e);
                }
            }
            logger.info("releasing triggers that a previous scheduler instance didn't release");
            ...
            if (lockedByInstanceId != null) {
                releaseLockedTriggers(lockedByInstanceId, RedisLabsTriggerState.ACQUIRED, RedisLabsTriggerState.WAITING);
                releaseLockedTriggers(lockedByInstanceId, RedisLabsTriggerState.BLOCKED, RedisLabsTriggerState.WAITING);
                releaseLockedTriggers(lockedByInstanceId, RedisLabsTriggerState.PAUSED_BLOCKED, RedisLabsTriggerState.PAUSED);
    ...
    public void storeJobAndTrigger(JobDetail newJob, OperableTrigger newTrigger)
        ...
        try {
            ...
            logger.error("could not store job: " + jobHashKey + " and trigger: " + triggerHashKey, ex);
    ...
    public void storeJob(JobDetail newJob, boolean replaceExisting)
        ...
        try {
            ...
            logger.warn(jobHashKey + " already exists", ex);
            throw ex;
        } catch (Exception ex) {
            logger.error("could not store job: " + jobHashKey, ex);
            throw new JobPersistenceException(ex.getMessage(), ex.getCause());
        } finally {
            jedis.close();
            unlock();
    ...
    public void storeJobsAndTriggers(
        ...
        try {
            ...
            logger.error("could not store jobs and triggers", ex);
            throw new JobPersistenceException(ex.getMessage(), ex.getCause());
        } finally {
            jedis.close();
            unlock();
    ...
    public boolean removeJob(JobKey jobKey) throws JobPersistenceException {
        ...
        try {
            ...
            logger.error("could not remove job: " + jobHashKey, ex);
            throw new JobPersistenceException(ex.getMessage(), ex.getCause());
        } finally {
            jedis.close();
            unlock();
    ...
    public JobDetail retrieveJob(JobKey jobKey) throws JobPersistenceException {
        ...
        try {
            ...
            logger.error("could not retrieve job: " + jobHashkey, ex);
            throw new JobPersistenceException(ex.getMessage(), ex.getCause());
        } finally {
            jedis.close();
            unlock();
    ...
    public void storeTrigger(OperableTrigger newTrigger, boolean replaceExisting)
        ...
        try {
            ...
            logger.warn(triggerHashKey + " already exists", ex);
            throw ex;
        } catch (Exception ex) {
            logger.error("could not store trigger: " + triggerHashKey, ex);
            throw new JobPersistenceException(ex.getMessage(), ex.getCause());
        } finally {
            jedis.close();
            unlock();
    ...
    private void storeTrigger(OperableTrigger newTrigger, boolean replaceExisting, Jedis jedis)
        ...
        if (jedis.sismember(PAUSED_TRIGGER_GROUPS_SET, triggerGroupSetKey) || jedis.sismember(PAUSED_JOB_GROUPS_SET, createJobGroupSetKey(newTrigger.getJobKey().getGroup()))) {
            ...
            if (jedis.sismember(BLOCKED_JOBS_SET, jobHashkey))
                setTriggerState(RedisLabsTriggerState.PAUSED_BLOCKED, (double)nextFireTime, triggerHashKey);
            else
                setTriggerState(RedisLabsTriggerState.PAUSED, (double)nextFireTime, triggerHashKey);
        } else if (newTrigger.getNextFireTime() != null) {
            setTriggerState(RedisLabsTriggerState.WAITING, (double)newTrigger.getNextFireTime().getTime(), triggerHashKey);
        }
    }
    ...
    public boolean removeTrigger(TriggerKey triggerKey)
        ...
        try {
            ...
            logger.error("could not remove trigger: " + triggerHashKey, ex);
            throw new JobPersistenceException(ex.getMessage(), ex.getCause());
        } finally {
            jedis.close();
            unlock();
     ...
     * @throws JobPersistenceException
     */
    private long setTriggerState(RedisLabsTriggerState state, double score, String triggerHashKey) throws JobPersistenceException {
        long success = -1;
        Jedis jedis = pool.getResource();
        try {
            ...
            logger.error("could not set state " + state.toString() + " for trigger: " + triggerHashKey);
            throw new JobPersistenceException(ex.getMessage(), ex.getCause());
        } finally {
            jedis.close();
        }
    ...
    private long unsetTriggerState(String triggerHashKey) throws JobPersistenceException {
        ...
        try {
            ...
            p.zrem(RedisLabsTriggerState.WAITING.getKey(), triggerHashKey);
            p.zrem(RedisLabsTriggerState.PAUSED.getKey(), triggerHashKey);
            p.zrem(RedisLabsTriggerState.BLOCKED.getKey(), triggerHashKey);
            p.zrem(RedisLabsTriggerState.PAUSED_BLOCKED.getKey(), triggerHashKey);
            p.zrem(RedisLabsTriggerState.ACQUIRED.getKey(), triggerHashKey);
            p.zrem(RedisLabsTriggerState.COMPLETED.getKey(), triggerHashKey);
            p.zrem(RedisLabsTriggerState.ERROR.getKey(), triggerHashKey);
            ...
            removed = -1L;
            logger.error("could not unset state for trigger: " + triggerHashKey);
            throw new JobPersistenceException(ex.getMessage(), ex.getCause());
        } finally {
            jedis.close();
        }
    ...
    public boolean replaceTrigger(TriggerKey triggerKey,
        ...
        try {
            ...
            logger.error("could not replace trigger: " + triggerHashKey, ex);
            throw new JobPersistenceException(ex.getMessage(), ex.getCause());
        } finally {
            jedis.close();
            unlock();
    ...
    public OperableTrigger retrieveTrigger(TriggerKey triggerKey) throws JobPersistenceException {
        ...
        try {
            ...
            logger.error("could not retrieve trigger: " + triggerHashKey, ex);
            throw new JobPersistenceException(ex.getMessage(), ex.getCause());
        } finally {
            jedis.close();
            unlock();
    ...
    private OperableTrigger retrieveTrigger(TriggerKey triggerKey, Jedis jedis) throws JobPersistenceException {
        ...
        if (!jedis.exists(triggerHashKey)) {
            logger.debug("trigger does not exist for key: " + triggerHashKey);
            return null;
        }
    ...
    private OperableTrigger toOperableTrigger(TriggerKey triggerKey, Map<String, String> trigger) {
        if (TRIGGER_TYPE_SIMPLE.equals(trigger.get(TRIGGER_TYPE))) {
            ...
            try {
                ...
                logger.warn("could not parse cron_expression: " + trigger.get(CRON_EXPRESSION) + " for trigger: " + createTriggerHashKey(triggerKey.getGroup(), triggerKey.getName()));
            }
    ...
    public boolean checkExists(JobKey jobKey) throws JobPersistenceException {
        ...
        try {
            ...
            logger.error("could not check if job: " + jobHashKey + " exists", ex);
            throw new JobPersistenceException(ex.getMessage(), ex.getCause());
        } finally {
            jedis.close();
        }
    ...
    public boolean checkExists(TriggerKey triggerKey)
        ...
        try {
            ...
            logger.error("could not check if trigger: " + triggerHashKey + " exists", ex);
            throw new JobPersistenceException(ex.getMessage(), ex.getCause());
        } finally {
            jedis.close();
        }
    ...
    public void clearAllSchedulingData() throws JobPersistenceException {
        ...
        try {
            ...
            logger.debug("all scheduling data cleared!");
        } catch (Exception ex) {
            logger.error("could not remove scheduling data", ex);
            throw new JobPersistenceException(ex.getMessage(), ex.getCause());
        } finally {
            jedis.close();
            unlock();
    ...
    public void storeCalendar(String name, Calendar calendar,
        ...
        try {
            ...
            logger.warn(calendarHashKey + " already exists");
            throw ex;
        } catch (Exception ex) {
            logger.error("could not store calendar: " + calendarHashKey, ex);
            throw new JobPersistenceException(ex.getMessage(), ex.getCause());
        } finally {
            jedis.close();
            unlock();
    ...
    private void storeCalendar(String name, Calendar calendar,
        ...
        if (updateTriggers) {
            ...
            for (String triggerHashKey : triggerHasjKeys) {
                ...
                long removed = jedis.zrem(RedisLabsTriggerState.WAITING.getKey(), triggerHashKey);
                trigger.updateWithNewCalendar(calendar, getMisfireThreshold());
                if (removed == 1)
                    setTriggerState(RedisLabsTriggerState.WAITING, (double)trigger.getNextFireTime().getTime(), triggerHashKey);
            }
        }
    }
    ...
    public boolean removeCalendar(String calName)
        ...
        try {
            ...
            logger.error("could not remove calendar: " + calendarHashKey, ex);
            throw new JobPersistenceException(ex.getMessage(), ex.getCause());
        } finally {
            jedis.close();
            unlock();
    ...
    public Calendar retrieveCalendar(String calName)
        ...
        try {
            ...
            logger.error("could not retrieve calendar: " + calendarHashKey, ex);
            throw new JobPersistenceException(ex.getMessage(), ex.getCause());
        } finally {
            jedis.close();
            unlock();
    ...
    private Calendar retrieveCalendar(String calName, Jedis jedis)
        ...
        try {
            ...
            Gson gson = new Gson();
            Class<Calendar> calClass = (Class<Calendar>) Class.forName(jedis.hget(calendarHashKey, CALENDAR_CLASS));
            calendar = gson.fromJson(jedis.hget(calendarHashKey, CALENDAR_SERIALIZED), calClass);
        } catch (ClassNotFoundException ex) {
            logger.warn("class not found for calendar: " + calName, ex);
        }
        return calendar;
    }
    ...
    public int getNumberOfJobs() throws JobPersistenceException {
        ...
        try {
            ...
            logger.error("could not get number of jobs", ex);
            throw new JobPersistenceException(ex.getMessage(), ex.getCause());
        } finally {
            jedis.close();
            unlock();
    ...
    public int getNumberOfTriggers() throws JobPersistenceException {
        ...
        try {
            ...
            logger.error("could not get number of triggers", ex);
            throw new JobPersistenceException(ex.getMessage(), ex.getCause());
        } finally {
            jedis.close();
            unlock();
    ...
    public Set<JobKey> getJobKeys(GroupMatcher<JobKey> matcher)
        ...
        try {
            ...
            logger.error("could not get job keys for job group: " + jobGroupSetKey, ex);
            throw new JobPersistenceException(ex.getMessage(), ex.getCause());
        } finally {
            jedis.close();
            unlock();
    ...
    public Set<TriggerKey> getTriggerKeys(GroupMatcher<TriggerKey> matcher)
        ...
        try {
            ...
            logger.error("could not get trigger keys for trigger group: " + triggerGroupSetKey, ex);
            throw new JobPersistenceException(ex.getMessage(), ex.getCause());
        } finally {
            jedis.close();
            unlock();
    ...
    public List<OperableTrigger> getTriggersForJob(JobKey jobKey)
        ...
        try {
            ...
            logger.error("could not get triggers for job_triggers: " + jobTriggerSetkey, ex);
            throw new JobPersistenceException(ex.getMessage(), ex.getCause());
        } finally {
            jedis.close();
            unlock();
    ...
    public TriggerState getTriggerState(TriggerKey triggerKey)
        ...
        try {
            ...
            if (jedis.zscore(RedisLabsTriggerState.PAUSED.getKey(), triggerHashKey) != null || jedis.zscore(RedisLabsTriggerState.PAUSED_BLOCKED.getKey(), triggerHashKey)!= null)
                return TriggerState.PAUSED;
            else if (jedis.zscore(RedisLabsTriggerState.BLOCKED.getKey(), triggerHashKey) != null)
                return TriggerState.BLOCKED;
            else if (jedis.zscore(RedisLabsTriggerState.WAITING.getKey(), triggerHashKey) != null || jedis.zscore(RedisLabsTriggerState.ACQUIRED.getKey(), triggerHashKey) != null)
                return TriggerState.NORMAL;
            else if (jedis.zscore(RedisLabsTriggerState.COMPLETED.getKey(), triggerHashKey) != null)
                return TriggerState.COMPLETE;
            else if (jedis.zscore(RedisLabsTriggerState.ERROR.getKey(), triggerHashKey) != null)
                return TriggerState.ERROR;
            else
                return TriggerState.NONE;
        } catch (Exception ex) {
            logger.error("could not get trigger state: " + triggerHashKey, ex);
            throw new JobPersistenceException(ex.getMessage(), ex.getCause());
        } finally {
            jedis.close();
            unlock();
    ...
    public void pauseTrigger(TriggerKey triggerKey)
        ...
        try {
            ...
            logger.error("could not pause trigger: " + triggerHashKey, ex);
            throw new JobPersistenceException(ex.getMessage(), ex.getCause());
        } finally {
            jedis.close();
            unlock();
    ...
    private void pauseTrigger(TriggerKey triggerKey, Jedis jedis) throws JobPersistenceException {
        ...
        if (jedis.zscore(RedisLabsTriggerState.COMPLETED.getKey(), triggerHashKey) != null)
            return;
        ...
        if (jedis.zscore(RedisLabsTriggerState.BLOCKED.getKey(), triggerHashKey) != null)
            setTriggerState(RedisLabsTriggerState.PAUSED_BLOCKED, (double)nextFireTime, triggerHashKey);
        else
            setTriggerState(RedisLabsTriggerState.PAUSED, (double)nextFireTime, triggerHashKey);
    }
    ...
    public Collection<String> pauseTriggers(GroupMatcher<TriggerKey> matcher)
        ...
        try {
            ...
            logger.error("could not pause triggers group: " + triggerGroupSetKey, ex);
            throw new JobPersistenceException(ex.getMessage(), ex.getCause());
        } finally {
            jedis.close();
            unlock();
    ...
    public void pauseJob(JobKey jobKey) throws JobPersistenceException {
        ...
        try {
            ...
            logger.error("could not pause job: " + jobHashKey, ex);
            throw new JobPersistenceException(ex.getMessage(), ex.getCause());
        } finally {
            jedis.close();
            unlock();
    ...
    public Collection<String> pauseJobs(GroupMatcher<JobKey> groupMatcher)
        ...
        try {
            ...
            logger.error("could not pause job group: " + jobGroupSetKey, ex);
            throw new JobPersistenceException(ex.getMessage(), ex.getCause());
        } finally {
            jedis.close();
            unlock();
    ...
    public void resumeTrigger(TriggerKey triggerKey)
        ...
        try {
            ...
            logger.error("could not resume trigger: " + triggerHashKey, ex);
            throw new JobPersistenceException(ex.getMessage(), ex.getCause());
        } finally {
            jedis.close();
            unlock();
    ...
    private void resumeTrigger(OperableTrigger trigger, Jedis jedis) throws JobPersistenceException {
        ...
        if (nextFireTime != null) {
            if (jedis.sismember(BLOCKED_JOBS_SET, jobHashKey))
                setTriggerState(RedisLabsTriggerState.BLOCKED, (double)nextFireTime.getTime(), triggerHashKey);
            else
                setTriggerState(RedisLabsTriggerState.WAITING, (double)nextFireTime.getTime(), triggerHashKey);
        }
    }
    ...
    public Set<String> getPausedTriggerGroups() throws JobPersistenceException {
        ...
        try {
            ...
            logger.error("could not get paused trigger groups", ex);
            throw new JobPersistenceException(ex.getMessage(), ex.getCause());
        } finally {
            jedis.close();
            unlock();
    ...
    public void resumeJob(JobKey jobKey) throws JobPersistenceException {
        ...
        try {
            ...
            logger.error("could not resume job: " + jobHashKey, ex);
            throw new JobPersistenceException(ex.getMessage(), ex.getCause());
        } finally {
            jedis.close();
            unlock();
    ...
    public Collection<String> resumeJobs(GroupMatcher<JobKey> matcher)
        ...
        try {
            ...
            logger.error("could not resume job group: " + jobGroupSetKey, ex);
            throw new JobPersistenceException(ex.getMessage(), ex.getCause());
        } finally {
            jedis.close();
            unlock();
    ...
    public void pauseAll() throws JobPersistenceException {
        ...
        try {
            ...
            logger.error("could not pause all triggers", ex);
            throw new JobPersistenceException(ex.getMessage(), ex.getCause());
        } finally {
            jedis.close();
            unlock();
    ...
    public void resumeAll() throws JobPersistenceException {
        ...
        try {
            ...
            logger.error("could not resume all triggers", ex);
            throw new JobPersistenceException(ex.getMessage(), ex.getCause());
        } finally {
            jedis.close();
            unlock();
    ...
    public List<OperableTrigger> acquireNextTriggers(long noLaterThan,
        ...
        try {
            ...
            while (retry) {
                ...
                Set<Tuple> triggerTuples = jedis.zrangeByScoreWithScores(RedisLabsTriggerState.WAITING.getKey(), 0d, (double)(noLaterThan + timeWindow), 0, maxCount);
                for (Tuple triggerTuple : triggerTuples) {
                    ...
                    setTriggerState(RedisLabsTriggerState.ACQUIRED, triggerTuple.getScore(), triggerTuple.getElement());
                    acquiredTriggers.add(trigger);
                    logger.debug("trigger: " + triggerTuple.getElement() + " acquired");
                }
            }
        } catch(Exception ex) {
            logger.error("could not acquire next triggers", ex);
            throw new JobPersistenceException(ex.getMessage(), ex.getCause());
        } finally {
            jedis.close();
            unlock();
    ...
    private void releaseTriggersCron() {
        if (this.releaseTriggersInterval > 0) {
            ...
            if ((System.currentTimeMillis() - lastTriggersReleseTime) > (this.releaseTriggersInterval)) {
                releaseOrphanedTriggers(RedisLabsTriggerState.ACQUIRED, RedisLabsTriggerState.WAITING);
                releaseOrphanedTriggers(RedisLabsTriggerState.BLOCKED, RedisLabsTriggerState.WAITING);
                releaseOrphanedTriggers(RedisLabsTriggerState.PAUSED_BLOCKED, RedisLabsTriggerState.PAUSED);
    ...
    private long getLastTriggersReleaseTime() {
        ...
        try {
            ...
            logger.error("could not get last trigger release time");
        } finally {
            jedis.close();
        }
    ...
    private void setLastTriggersReleaseTime(String time) {
        ...
        try {
            ...
            logger.error("could not get last triggerrelease time");
        } finally {
            jedis.close();
        }
    }
     ...
     */
    private void releaseOrphanedTriggers(RedisLabsTriggerState currentState, RedisLabsTriggerState newState) {
        ...
        try {
            ...
            logger.error("could not release orphaned triggers from: " + currentState, ex);
        } finally {
            jedis.close();
            unlock();
        }
     ...
     */
    private void releaseLockedTriggers(String lockedByInstanceId, RedisLabsTriggerState currentState, RedisLabsTriggerState newState) {
        Jedis jedis = pool.getResource();
        try {
            lock();
            ...
            for (Tuple triggerTuple : triggerTuples) {
                if (lockedByInstanceId != null && lockedByInstanceId.equals(jedis.hget(triggerTuple.getElement(), LOCKED_BY))) {
                    logger.debug("releasing trigger: " + triggerTuple.getElement() + " from: " + currentState.getKey());
                    setTriggerState(newState, triggerTuple.getScore(), triggerTuple.getElement());
                }
            }
        } catch (Exception ex) {
            logger.error("could not release locked triggers in: " + currentState.getKey(), ex);
        } finally {
            jedis.close();
            unlock();
        }
    ...
    private String readInstanceId(File instanceIdFile) {
        ...
        try {
            ...
            logger.error("could not read previous scheduler instance id", ex);
        } finally {
            try {
                if (isr != null) {
                    isr.close();
                ...
                logger.warn("could not close file reader", ex);
            }
        }
    ...
    private void writeInstanceId(File instanceIdFile) {
        ...
        try {
            ...
            logger.error("could not write new scheduler instance id", ex);
        } finally {
            try {
                if (out != null)
                    out.close();
            } catch (IOException ex) {
                logger.warn("could not close file writer", ex);
            }
        }
    }
    ...
    private void releaseBlockedJobs(String blockedByInstanceId) {
        ...
        try {
            ...
            logger.error("could not release blocked jobs", ex);
        } finally {
            jedis.close();
            unlock();
        }
    ...
    private boolean isJobConcurrentExectionDisallowed(String jobClassName) {
        ...
        try {
            ...
            logger.error("could not determine whether class: " + jobClassName + " is JobConcurrentExectionDisallowed annotated");
        }
    ...
    private boolean isPersistJobDataAfterExecution(String jobClassName) {
        ...
        try {
            ...
            logger.error("could not determine whether class: " + jobClassName + " is PersistJobDataAfterExecution annotated");
        }
    ...
    public void releaseAcquiredTrigger(OperableTrigger trigger) {
        ...
        try {
            ...
            String triggerHashKey = createTriggerHashKey(trigger.getKey().getGroup(), trigger.getKey().getName());
            if (jedis.zscore(RedisLabsTriggerState.ACQUIRED.getKey(), triggerHashKey) != null) {
                if (trigger.getNextFireTime() != null)
                    setTriggerState(RedisLabsTriggerState.WAITING, (double)trigger.getNextFireTime().getTime(), triggerHashKey);
                else
                    unsetTriggerState(triggerHashKey);
            ...
            logger.error("could not release acquired triggers", ex);
        } finally {
            jedis.close();
            unlock();
        }
    ...
    public List<TriggerFiredResult> triggersFired(List<OperableTrigger> triggers)
        ...
        try {
            ...
            for (OperableTrigger trigger : triggers) {
                String triggerHashKey = createTriggerHashKey(trigger.getKey().getGroup(), trigger.getKey().getName());
                logger.debug("trigger: " + triggerHashKey + " fired");
                ...
                if (isJobConcurrentExectionDisallowed(jedis.hget(jobHashKey, JOB_CLASS))) {
                    ...
                    for (String nonConcurrentTriggerHashKey : nonConcurrentTriggerHashKeys) {
                        Double score = jedis.zscore(RedisLabsTriggerState.WAITING.getKey(), nonConcurrentTriggerHashKey);
                        if (score != null) {
                            setTriggerState(RedisLabsTriggerState.BLOCKED, score, nonConcurrentTriggerHashKey);
                        } else {
                            score = jedis.zscore(RedisLabsTriggerState.PAUSED.getKey(), nonConcurrentTriggerHashKey);
                            if (score != null)
                                setTriggerState(RedisLabsTriggerState.PAUSED_BLOCKED, score, nonConcurrentTriggerHashKey);
                        }
                    }
    ...
    public void triggeredJobComplete(OperableTrigger trigger,
        ...
        String triggerHashKey = createTriggerHashKey(trigger.getKey().getGroup(), trigger.getKey().getName());
        logger.debug("job: " + jobHashKey + " completed");
        Jedis jedis = pool.getResource();
        try {
            lock();
            ...
            if (jedis.exists(jobHashKey)) { // checking that the job wasn't deleted during the execution
                ...
                if (isJobConcurrentExectionDisallowed(jobClassName)) {
                    ...
                    for (String nonConcurrentTriggerHashkey : nonConcurrentTriggerHashKeys) {
                        Double score = jedis.zscore(RedisLabsTriggerState.BLOCKED.getKey(), nonConcurrentTriggerHashkey);
                        if (score != null) {
                            setTriggerState(RedisLabsTriggerState.WAITING, score, nonConcurrentTriggerHashkey);
                        } else {
                            score = jedis.zscore(RedisLabsTriggerState.PAUSED_BLOCKED.getKey(), nonConcurrentTriggerHashkey);
                            if (score != null)
                                setTriggerState(RedisLabsTriggerState.PAUSED, score, nonConcurrentTriggerHashkey);
                        }
                    }
            ...
            if (jedis.exists(triggerHashKey)) { // checking that the trigger wasn't deleted during the execution
                ...
                if (triggerInstCode == CompletedExecutionInstruction.DELETE_TRIGGER) {
                    ...
                    setTriggerState(RedisLabsTriggerState.COMPLETED, (double)System.currentTimeMillis(), triggerHashKey);
                    signaler.signalSchedulingChange(0L);
                } else if(triggerInstCode == CompletedExecutionInstruction.SET_TRIGGER_ERROR) {
                    logger.debug("trigger: " + triggerHashKey + " ended with error");
                    double score = trigger.getNextFireTime() != null ? (double)trigger.getNextFireTime().getTime() : 0D;
                    setTriggerState(RedisLabsTriggerState.ERROR, score, triggerHashKey);
                    signaler.signalSchedulingChange(0L);
                    ...
                    for (String errorTriggerHashKey : errorTriggerHashkeys) {
                        ...
                        setTriggerState(RedisLabsTriggerState.ERROR, score, errorTriggerHashKey);
                    }
                    signaler.signalSchedulingChange(0L);
                    ...
                    for (String completedTriggerHashKey : comletedTriggerHashkeys) {
                        setTriggerState(RedisLabsTriggerState.COMPLETED, (double)System.currentTimeMillis(), completedTriggerHashKey);
                    }
                    signaler.signalSchedulingChange(0L);
                }
            }
        } catch(Exception ex) {
            logger.error("could not handle triggegered job completion", ex);
        } finally {
            jedis.close();
            unlock();
        }
    ...
    protected boolean applyMisfire(OperableTrigger trigger, Jedis jedis) throws JobPersistenceException {
        ...
        if (trigger.getNextFireTime() == null) { // Trigger completed
            setTriggerState(RedisLabsTriggerState.COMPLETED, (double)System.currentTimeMillis(), createTriggerHashKey(trigger.getKey().getGroup(), trigger.getKey().getName()));
    ...
    private void keepAlive() {
        ...
        try {
            ...
            logger.error("could not keep alive");
        } finally {
            jedis.close();
        }
    }
    ...
    private void lock() throws JobPersistenceException {
        ...
        try {
            ...
            logger.error("could not lock mutex", ex);
            throw new JobPersistenceException(ex.getMessage(), ex.getCause());
        } finally {
            jedis.close();
        }
    ...
    private void unlock() {
        ...
        try {
            ...
            logger.error("could not unlock mutex", ex);
        } finally {
            jedis.close();
        }
    }
    ...
    class UnlockListener extends JedisPubSub {
        ...
        public void onMessage(String channel, String message) {
            logger.debug("message recieved: " + message + " on channel: " + channel);
        ...
        public void onPMessage(String pattern, String channel, String message) {
            logger.debug("pmessage recieved: " + message + " on channel: " + channel + ", pattern: " + pattern);
        }
        ...
        public void onUnsubscribe(String channel, int subscribedChannels) {
            logger.debug("unsubscribing to channel: " + channel);
        }

        @Override
        public void onPUnsubscribe(String pattern, int subscribedChannels) {
            logger.debug("punsubscribing to channels pattern: " + pattern + ", subscribed channels: " + subscribedChannels);
        }

        @Override
        public void onPSubscribe(String pattern, int subscribedChannels) {
            logger.debug("psubscribing to channels pattern: " + pattern + ", subscribed channels: " + subscribedChannels);
        }
    }
}
