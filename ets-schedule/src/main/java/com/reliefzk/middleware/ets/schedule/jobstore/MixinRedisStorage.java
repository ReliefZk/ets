...
public class MixinRedisStorage extends MixinAbstractRedisStorage<Jedis> {
    private static final Logger logger = LoggerFactory.getLogger(MixinRedisStorage.class);

    public MixinRedisStorage(MixinRedisJobStoreSchema redisSchema, ObjectMapper mapper, SchedulerSignaler signaler, String schedulerInstanceId, int lockTimeout) {
    ...
    public void storeTrigger(OperableTrigger trigger, boolean replaceExisting, Jedis jedis) throws JobPersistenceException {
        ...
        if(triggerPausedResponse.get() || jobPausedResponse.get()){
            ...
            if(jedis.sismember(redisSchema.blockedJobsSet(), jobHashKey)){
                setTriggerState(MixinRedisTriggerState.PAUSED_BLOCKED, (double) nextFireTime, triggerHashKey, jedis);
            }
            else{
                setTriggerState(MixinRedisTriggerState.PAUSED, (double) nextFireTime, triggerHashKey, jedis);
            }
        }
        else if(trigger.getNextFireTime() != null){
            setTriggerState(MixinRedisTriggerState.WAITING, (double) trigger.getNextFireTime().getTime(), triggerHashKey, jedis);
        }
    }
    ...
    public boolean unsetTriggerState(final String triggerHashKey, Jedis jedis) throws JobPersistenceException {
        ...
        Pipeline pipe = jedis.pipelined();
        List<Response<Long>> responses = new ArrayList<Response<Long>>(MixinRedisTriggerState.values().length);
    ...
    public void storeCalendar(String name, Calendar calendar, boolean replaceExisting, boolean updateTriggers, Jedis jedis) throws JobPersistenceException{
        ...
        if(updateTriggers){
            ...
            for (String triggerHashKey : triggerHashKeys) {
                ...
                long removed = jedis.zrem(redisSchema.triggerStateKey(MixinRedisTriggerState.WAITING), triggerHashKey);
    ...
    public Set<JobKey> getJobKeys(GroupMatcher<JobKey> matcher, Jedis jedis){
        ...
        if(matcher.getCompareWithOperator() == StringMatcher.StringOperatorName.EQUALS){
            ...
            List<Response<Set<String>>> jobGroups = new ArrayList<Response<Set<String>>>();
            Pipeline pipe = jedis.pipelined();
    ...
    public Set<TriggerKey> getTriggerKeys(GroupMatcher<TriggerKey> matcher, Jedis jedis){
        ...
        if(matcher.getCompareWithOperator() == StringMatcher.StringOperatorName.EQUALS){
            ...
            List<Response<Set<String>>> triggerGroups = new ArrayList<Response<Set<String>>>();
            Pipeline pipe = jedis.pipelined();
    ...
    public Trigger.TriggerState getTriggerState(TriggerKey triggerKey, Jedis jedis){
        final String triggerHashKey = redisSchema.triggerHashKey(triggerKey);
        Pipeline pipe = jedis.pipelined();
        Map<MixinRedisTriggerState, Response<Double>> scores = new HashMap<MixinRedisTriggerState, Response<Double>>(MixinRedisTriggerState.values().length);
        for (MixinRedisTriggerState redisTriggerState : MixinRedisTriggerState.values()) {
            scores.put(redisTriggerState, pipe.zscore(redisSchema.triggerStateKey(redisTriggerState), triggerHashKey));
        }
        pipe.sync();
        for (Map.Entry<MixinRedisTriggerState, Response<Double>> entry : scores.entrySet()) {
    ...
    public void pauseTrigger(TriggerKey triggerKey, Jedis jedis) throws JobPersistenceException {
        ...
        Response<Double> completedScore = pipe.zscore(redisSchema.triggerStateKey(MixinRedisTriggerState.COMPLETED), triggerHashKey);
        Response<String> nextFireTimeResponse = pipe.hget(triggerHashKey, TRIGGER_NEXT_FIRE_TIME);
        Response<Double> blockedScore = pipe.zscore(redisSchema.triggerStateKey(MixinRedisTriggerState.BLOCKED), triggerHashKey);
        pipe.sync();
    ...
    public Collection<String> pauseTriggers(GroupMatcher<TriggerKey> matcher, Jedis jedis) throws JobPersistenceException {
        Set<String> pausedTriggerGroups = new HashSet<String>();
        if(matcher.getCompareWithOperator() == StringMatcher.StringOperatorName.EQUALS){
            final String triggerGroupSetKey = redisSchema.triggerGroupSetKey(new TriggerKey("", matcher.getCompareToValue()));
            ...
            Map<String, Response<Set<String>>> triggerGroups = new HashMap<String, Response<Set<String>>>();
            Pipeline pipe = jedis.pipelined();
            for (final String triggerGroupSetKey : jedis.smembers(redisSchema.triggerGroupsSet())) {
                if(matcher.getCompareWithOperator().evaluate(redisSchema.triggerGroup(triggerGroupSetKey), matcher.getCompareToValue())){
    ...
    public Collection<String> pauseJobs(GroupMatcher<JobKey> groupMatcher, Jedis jedis) throws JobPersistenceException {
        ...
        if(groupMatcher.getCompareWithOperator() == StringMatcher.StringOperatorName.EQUALS){
            ...
            Map<String, Response<Set<String>>> jobGroups = new HashMap<String, Response<Set<String>>>();
            Pipeline pipe = jedis.pipelined();
            for (final String jobGroupSetKey : jedis.smembers(redisSchema.jobGroupsSet())) {
    ...
    public void resumeTrigger(TriggerKey triggerKey, Jedis jedis) throws JobPersistenceException {
        ...
        Response<Double> isPaused = pipe.zscore(redisSchema.triggerStateKey(MixinRedisTriggerState.PAUSED), triggerHashKey);
        Response<Double> isPausedBlocked = pipe.zscore(redisSchema.triggerStateKey(MixinRedisTriggerState.PAUSED_BLOCKED), triggerHashKey);
        pipe.sync();
        ...
        if(nextFireTime != null){
            if(jedis.sismember(redisSchema.blockedJobsSet(), jobHashKey)){
                setTriggerState(MixinRedisTriggerState.BLOCKED, (double) nextFireTime.getTime(), triggerHashKey, jedis);
            }
            else{
                setTriggerState(MixinRedisTriggerState.WAITING, (double) nextFireTime.getTime(), triggerHashKey, jedis);
            }
        }
    ...
    public Collection<String> resumeTriggers(GroupMatcher<TriggerKey> matcher, Jedis jedis) throws JobPersistenceException {
        Set<String> resumedTriggerGroups = new HashSet<String>();
        if (matcher.getCompareWithOperator() == StringMatcher.StringOperatorName.EQUALS) {
            final String triggerGroupSetKey = redisSchema.triggerGroupSetKey(new TriggerKey("", matcher.getCompareToValue()));
            Pipeline pipe = jedis.pipelined();
    ...
    public List<TriggerFiredResult> triggersFired(List<OperableTrigger> triggers, Jedis jedis) throws JobPersistenceException, ClassNotFoundException {
        ...
        for (OperableTrigger trigger : triggers) {
            ...
            Response<Double> triggerAcquiredResponse = pipe.zscore(redisSchema.triggerStateKey(MixinRedisTriggerState.ACQUIRED), triggerHashKey);
            pipe.sync();
            ...
            if(isJobConcurrentExecutionDisallowed(job.getJobClass())){
                ...
                for (String nonConcurrentTriggerHashKey : jedis.smembers(jobTriggerSetKey)) {
                    Double score = jedis.zscore(redisSchema.triggerStateKey(MixinRedisTriggerState.WAITING), nonConcurrentTriggerHashKey);
                    if(score != null){
                        setTriggerState(MixinRedisTriggerState.BLOCKED, score, nonConcurrentTriggerHashKey, jedis);
                    }
                    else{
                        score = jedis.zscore(redisSchema.triggerStateKey(MixinRedisTriggerState.PAUSED), nonConcurrentTriggerHashKey);
                        if(score != null){
                            setTriggerState(MixinRedisTriggerState.PAUSED_BLOCKED, score, nonConcurrentTriggerHashKey, jedis);
                        }
                    }
                }
                pipe = jedis.pipelined();
            ...
            if(trigger.getNextFireTime() != null){
                ...
                setTriggerState(MixinRedisTriggerState.WAITING, (double) nextFireTime, triggerHashKey, jedis);
            }
            else{
    ...
    public void triggeredJobComplete(OperableTrigger trigger, JobDetail jobDetail, Trigger.CompletedExecutionInstruction triggerInstCode, Jedis jedis) throws JobPersistenceException, ClassNotFoundException {
        ...
        if(jedis.exists(jobHashKey)) {
            ...
            if (isJobConcurrentExecutionDisallowed(jobDetail.getJobClass())) {
                ...
                for (String nonConcurrentTriggerHashKey : jedis.smembers(jobTriggersSetKey)) {
                    Double score = jedis.zscore(redisSchema.triggerStateKey(MixinRedisTriggerState.BLOCKED), nonConcurrentTriggerHashKey);
                    if (score != null) {
                        setTriggerState(MixinRedisTriggerState.WAITING, score, nonConcurrentTriggerHashKey, jedis);
                    }
                    else {
                        score = jedis.zscore(redisSchema.triggerStateKey(MixinRedisTriggerState.PAUSED_BLOCKED), nonConcurrentTriggerHashKey);
                        if (score != null) {
                            setTriggerState(MixinRedisTriggerState.PAUSED, score, nonConcurrentTriggerHashKey, jedis);
                        }
                    }
                }
        ...
        if(jedis.exists(triggerHashKey)){
            ...
            if(triggerInstCode == Trigger.CompletedExecutionInstruction.DELETE_TRIGGER){
                ...
                setTriggerState(MixinRedisTriggerState.COMPLETED, (double) System.currentTimeMillis(), triggerHashKey, jedis);
                signaler.signalSchedulingChange(0L);
            }
                ...
                setTriggerState(MixinRedisTriggerState.ERROR, score, triggerHashKey, jedis);
                signaler.signalSchedulingChange(0L);
            }
                ...
                for (String errorTriggerHashKey : jedis.smembers(jobTriggersSetKey)) {
                    ...
                    setTriggerState(MixinRedisTriggerState.ERROR, score, errorTriggerHashKey, jedis);
                }
                signaler.signalSchedulingChange(0L);
            }
                ...
                for (String completedTriggerHashKey : jedis.smembers(jobTriggerSetKey)) {
                    setTriggerState(MixinRedisTriggerState.COMPLETED, (double) System.currentTimeMillis(), completedTriggerHashKey, jedis);
                }
                signaler.signalSchedulingChange(0L);
            }
        }
