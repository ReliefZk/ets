 ...
 * 7/16/2014
 */
public abstract class MixinAbstractRedisStorage<T extends JedisCommands> {

    private static final Logger logger = LoggerFactory.getLogger(MixinAbstractRedisStorage.class);
    ...
    protected final MixinRedisJobStoreSchema redisSchema;
    ...
    public MixinAbstractRedisStorage(MixinRedisJobStoreSchema redisSchema, ObjectMapper mapper, SchedulerSignaler signaler, String schedulerInstanceId, int lockTimeout) {
    ...
    /**
     * Clear (delete!) all scheduling data - all {@link Job}s, {@link Trigger}s {@link java.util.Calendar}s.
     * @param jedis a thread-safe Redis connection
     */
    ...
    /**
     * Convert a {@link org.quartz.JobDataMap} to a {@link HashMap} with String keys and values
     ...
     */
    public boolean setTriggerState(final MixinRedisTriggerState state, final double score, final String triggerHashKey, T jedis) throws JobPersistenceException{
    ...
    public List<String> getJobGroupNames(T jedis){
        ...
        List<String> groups = new ArrayList<String>(groupsSet.size());
    ...
    public List<String> getCalendarNames(T jedis){
        ...
        List<String> calendars = new ArrayList<String>(calendarsSet.size());
    ...
    protected boolean applyMisfire(OperableTrigger trigger, T jedis) throws JobPersistenceException {
        ...
        if(trigger.getNextFireTime() == null){
            setTriggerState(MixinRedisTriggerState.COMPLETED, (double) System.currentTimeMillis(), redisSchema.triggerHashKey(trigger.getKey()), jedis);
    ...
    public Set<String> getPausedTriggerGroups(T jedis){
        ...
        Set<String> names = new HashSet<String>(triggerGroupSetKeys.size());
     ...
     * @param jedis a thread-safe Redis connection
     */
    protected void releaseOrphanedTriggers(MixinRedisTriggerState currentState, MixinRedisTriggerState newState, T jedis) throws JobPersistenceException {
    ...
    protected void releaseTriggersCron(T jedis) throws JobPersistenceException {
        if(System.currentTimeMillis() - getLastTriggersReleaseTime(jedis) > TRIGGER_LOCK_TIMEOUT){
            ...
            releaseOrphanedTriggers(MixinRedisTriggerState.ACQUIRED, MixinRedisTriggerState.WAITING, jedis);
            releaseOrphanedTriggers(MixinRedisTriggerState.BLOCKED, MixinRedisTriggerState.WAITING, jedis);
            releaseOrphanedTriggers(MixinRedisTriggerState.PAUSED_BLOCKED, MixinRedisTriggerState.PAUSED, jedis);
    ...
    public List<OperableTrigger> acquireNextTriggers(long noLaterThan, int maxCount, long timeWindow, T jedis) throws JobPersistenceException, ClassNotFoundException {
        ...
        do{
            ...
            for (Tuple triggerTuple : jedis.zrangeByScoreWithScores(redisSchema.triggerStateKey(MixinRedisTriggerState.WAITING), 0, (double) (noLaterThan + timeWindow), 0, maxCount)) {
                ...
                setTriggerState(MixinRedisTriggerState.ACQUIRED, triggerTuple.getScore(), triggerTuple.getElement(), jedis);
    ...
    public void releaseAcquiredTrigger(OperableTrigger trigger, T jedis) throws JobPersistenceException {
        ...
        if(jedis.zscore(redisSchema.triggerStateKey(MixinRedisTriggerState.ACQUIRED), triggerHashKey) != null){
            if(trigger.getNextFireTime() != null){
